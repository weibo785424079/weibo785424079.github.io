...
const Component = ({ onChange, format, value, style, element, ...rest }, ref) => {
  ...
  return (
    <div ref={ref} className="site-component-color-picker" style={style}>
      <ColorPicker
        type={format}
        animation="slide-up"
        color={(value && (value as any).slice(0, 7)) || '#ffffff'}
      ...
      {rest.readonly ? (
        ...
        <Input placeholder="#ffffff" disabled={rest.disabled} value={value} onChange={onInputChange} />
      )}
    </div>
  );
};
